...
class Game:
    def __init__(self):
        ...
        self.frame = self.gameWin.makeFrame(200, 100)
    
    def menu(self):
     
        menuFrame = self.gameWin.makeFrame(200, 100) # make sure the packing doesnt effect the other button and label
        
        titleLabel = self.gameWin.makeLabel("Choose Your Own Adventure")
        titleLabel.pack(side="top", padx=10, pady=10)
        startButton = self.gameWin.makeButton("Start", (lambda: self.intro(menuFrame)))
        startButton.pack(side="top", padx=10, pady=10)
        menuFrame.pack(side="top", padx=10, pady=10)
    
    def intro(self, frame):
        self.frame.destroy() # destroy the menu frame
        introFrame = self.gameWin.makeFrame(200, 100)
    
        nameLabel = self.gameWin.makeLabel("What is your name?")
        self.name = self.gameWin.makeInput()
        startButton =self.gameWin.makeButton("Start", (lambda: self.disclaimer(frame)))
    ...
    def disclaimer(self, frame):
        self.frame.destroy() # destroy the intro frame
        disclaimerFrame = self.gameWin.makeFrame(200, 100)
        
        disclaimerLabel = self.gameWin.makeLabel("**TEST GAME**\n This is a test game for the Choose Your Own Adventure game.\nIt is not the final game and is only for testing purposes.\nThe final game will have a proper storyline, choices, dialogs, and character animations.\n\n**CUTSCENE HERE**\n\nPress Enter to continue!")
        continueButton = self.gameWin.makeButton("Continue", (lambda: self.stage1(disclaimerFrame)))
    ...
    def stage1(self, frame):
        self.gameWin.disclaimerFrame.destroy()
        
        stage1Frame = self.gameWin.makeFrame(200, 100)
        stage1Label = self.gameWin.makeLabel(f"Hello {self.name}! You find yourself in a dark room with two doors. The first door is red, the second is white! Which door do you want to choose?")
        #redDoor = self.gameWin.makeButton("Red", (lambda: self.redDoor()))
        #whiteDoor = self.gameWin.makeButton("White", (lambda: self.stage2()))
    ...
    #     def redDoor(self):
    #         self.gameWin.stage1Frame.destroy() # destroy the stage 1 frame

    #         frame = self.gameWin.makeFrame(200, 100, 10, 10)
